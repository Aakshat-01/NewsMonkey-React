...
export default class News extends Component {
  ...
  async updateNews() {
    try {
      ...
      const apiBaseUrl = process.env.NODE_ENV === "production" ? "/api/gnews" : "https://gnews.io/api/v4";
      const url = `${apiBaseUrl}/top-headlines?country=${this.props.country}&lang=en&category=${this.props.category}&max=${this.props.pageSize}&page=${this.state.page}&apikey=${this.props.apiKey}`;
  ...
  fetchMoreData = async () => {
    try {
      ...
      const apiBaseUrl = process.env.NODE_ENV === "production" ? "/api/gnews" : "https://gnews.io/api/v4";
      const url = `${apiBaseUrl}/top-headlines?country=${this.props.country}&lang=en&category=${this.props.category}&max=${this.props.pageSize}&page=${nextPage}&apikey=${this.props.apiKey}`;
      let data = await fetch(url);
      let parsedData = await data.json();
